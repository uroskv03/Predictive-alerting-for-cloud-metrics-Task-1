...
INCIDENT_STEP = 200
CLASS_WEIGHT_INCIDENT = 1.0
#EPOCHS = 15
ALERT_THRESHOLD = 0.25  #0.5
W, H = 15, 5
N_STEPS = 2000

def generate_data(n_steps, step_freq):

    time = np.linspace(0, n_steps / 20, n_steps)
...
if __name__ == '__main__':
    ...
    model = RandomForestClassifier(
        n_estimators=100,
        max_depth=10,
        class_weight={0: 1.0, 1: CLASS_WEIGHT_INCIDENT},
    )

    print(f"Experiment started: Random Forest, Step={INCIDENT_STEP}, Weight={CLASS_WEIGHT_INCIDENT}")
    ...
    plt.plot(cpu[offset:][-view_limit:], label='CPU Utilization (AWS CloudWatch)', color='teal', alpha=0.7, linewidth=1.5)
    plt.plot(ram[offset:][-view_limit:], label='RAM Utilization (AWS CloudWatch)', color='orange', alpha=0.7, linewidth=1.5)
